...
(defclass nuclear-animation (paintable)
  ((drawbackground :initform nil
                   :accessor nuclear-animation-drawbackground)
   ...
   (progress :initform 0
             :accessor nuclear-animation-progress)
   ...
   (sourceid :initform 0
             :accessor nuclear-animation-sourceid))
  (:gname . "GdkNuclearAnimation")
  (:metaclass gobject:gobject-class))

(eval-when (:compile-toplevel :load-toplevel :execute)
  (export 'nuclear-animation))

(gobject:register-object-type-implementation "GdkNuclearAnimation"  ; name
                                             nuclear-animation      ; class
                                             "GObject"              ; parent
                                             ("GdkPaintable")       ; interfaces
                                             nil)                   ; properties

(defmethod initialize-instance :after ((obj nuclear-animation) &rest initargs)
  ;; Set the slot values from initargs
  (iter (for (slot value) on initargs by #'cddr)
        (cond ((eq slot :drawbackground)
               (setf (nuclear-animation-drawbackground obj) value))
              ((eq slot :progress)
               (setf (nuclear-animation-progress obj) value))
              ((eq slot :sourceid)
               (setf (nuclear-animation-sourceid obj) value))))
  ...
  (let ((sourceid (g:timeout-add 10
    ...
    (tg:finalize obj (lambda ()
                       (format t " FINALIZE for source : ~a~%" sourceid)
