...
(defmacro with-foreign-string-array ((strptr strs) &body body)
  (let ((i (gensym))
        (str (gensym)))
    `(let ((,strptr (foreign-alloc :pointer :count (1+ (length ,strs)))))
       (loop for ,i from 0
             for ,str in ,strs
             do (setf (mem-aref ,strptr :pointer ,i)
                      (foreign-string-alloc ,str)))
       (setf (mem-aref ,strptr :pointer (length ,strs)) (null-pointer))
       (unwind-protect
         (progn ,@body)
         (progn
           (loop for ,i from 0
                 repeat (1- (length ,strs))
                 do (foreign-string-free (mem-aref ,strptr :pointer ,i)))
           (foreign-string-free ,strptr))))))

(defun builder-add-objects-from-file (builder filename &rest args)
 #+liber-documentation
 "@version{#2022-9-13}
  @argument[builder]{a @class{gtk:builder} object}
  @argument[filename]{a string with the name of the file to parse}
  ...
  (with-foreign-string-array (strptr args)
    (with-g-error (err)
      (%builder-add-objects-from-file builder filename strptr err))))
